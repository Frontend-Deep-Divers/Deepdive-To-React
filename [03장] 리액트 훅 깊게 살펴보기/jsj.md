...
- useEffect
    ...
    - 특별한 기능을 통해 값의 변화를 관찰하는 것이 아니고 렌더링할 때마다 의존성에 있는 값을 보면서 이 의존성의 값이 이전과 다른게 하나라도 있으면 부수 효과를 실행하는 함수다.
    - 클린업 함수
...
- useReducer
    - useState와 비슷한 형태를 띠지만 좀 더 복잡한 상태 값을 미리 정의해놓은 시나리오에 따라 관리할 수 있다.
...
- 사용자 정의 훅
    ...
    - 리액트에서만 사용할 수 있는 방식이며 훅의 이름 앞에 반드시 use로 시작해야한다.
    - use로 시작하는 이름을 가져야 react-hooks/rules-of-hooks 의 도움을 받을 수 있고 리액트 훅의 규칙을 따를 수 있다.
    - 복잡하고 반복되는 로직은 사용자 정의 훅으로 간단하게 만들 수 있다.
- 고차 컴포넌트
    - 컴포넌트 자체의 로직을 재사용하기 위한 방법
    - 자바 스크립트의 일급 객체, 함수의 특징을 이용하므로 리액트가 아니고 자바스크립트 환경에서도 널리 쓰일 수 있다.
    - 컴포넌트 전체를 감쌀 수 있다는 점에서 사용자 정의 훅보다 더욱 큰 영향력을 컴포넌트에 미칠 수 있다. 또한, 단순히 값을 반환하거나 부수 효과를 실행하는 사용자 정의훅과는 다르게, 고차 컴포넌트는 컴포넌트의 결과물에 영향을 미칠 수 있는 다른 공통된 작업을 처리할 수 있다.
    - with로 시작하는 이름을 가져야한다. 강제 되는 사항은 아니지만 관습적인 이름이다.
- 사용해야하는 경우
    - 사용자 정의 훅
        - 리액트에서 제공하는 훅으로만 공통 로직을 격리할 수 있는 경우
        - 컴포넌트 전반에 걸쳐 동일한 로직으로 값을 제공하거나 특정한 훅의 작동을 취하게 하고 싶은 경우
        - 사용자 정의 훅 자체는 렌더링에 영향을 미치지 못하기 때문에 사용이 제한적이며, 단순히 값만 제공한다.
    - 고차 컴포넌트
        - 렌더링 결과물에까지 영향을 미쳐야 하는 경우 (중복 처리가 훅 + 컴포넌트 렌더링에도 일어나는 경우)
        - 허나 고차 컴포넌트가 많아질수록 복잡성이 기하급수적으로 증가하므로 신중하게 사용하자.